...
def analyze_repo(repo: RepoCandidate) -> AnalysisResult:
    ...
    if package_json:
        macos.append("安装 Node.js 18 或更新版本")
        windows.append("安装 Node.js 18 或更新版本，并确认 npm 可用")
        likely_commands.extend(["npm install", "npm run dev"])
    if requirements or language == "python":
        macos.append("安装 Python 3.11 或更新版本")
        windows.append("安装 Python 3.11 或更新版本，并勾选 Add python.exe to PATH")
        likely_commands.extend(["python -m venv .venv", "pip install -r requirements.txt"])
    if not likely_commands:
        likely_commands.append("先阅读 README 的 Installation 或 Quick Start 部分")
        notes.append("没有发现 package.json 或 requirements.txt，运行方式需要以 README 为准。")

    plain_summary = _plain_summary(repo, readme)
    difficulty = _difficulty_score(repo, readme, package_json, requirements)

    if _mentions_docker(readme):
        macos.append("可选安装 Docker Desktop")
        windows.append("可选安装 Docker Desktop，Windows 可能还需要启用 WSL2")
        likely_commands.append("docker compose up")
    if _needs_api_key(readme):
        notes.append("README 提到了 API key 或 token，运行前大概率需要注册第三方服务并配置环境变量。")
...
def _plain_summary(repo: RepoCandidate, readme: str) -> str:
    description = (repo.description or "").strip().rstrip(".")
    if description:
        return f"它像是一个帮你处理日常事务的小工具：{description}。"

    first_sentence = _first_readme_sentence(readme)
    if first_sentence:
        return f"它像是一个日常小工具：{first_sentence}"
